...
// Store-specific screen component
const InsideScoop = () => {
  const navigation = useNavigation();
  const storeData = stores.snacks.find(store => store.id === 3); // Replace with the correct store
  const { userRole } = useUserRole(); // Get user role from context
  ...
  return (
    <View style={styles.container}>
      <Image
        source={storeData?.localImage ? storeData.localImage : undefined}
        style={styles.storeImage}
      />
      <View style={styles.storeInfo}>
        <View style={styles.storeDetails}>
          <Text style={styles.category}>Snacks and Dessert</Text>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate(G05)} // Update with correct route name
        >
